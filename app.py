...
try:
    from database import get_cloud_market_prices_latest_dates
    core_tickers = ["DBC", "SPY", "QQQ", "SOXX", "QLD", "GLD", "TLT", "BITO", "JEPQ", "SGOV"]
    ticker_dates = get_cloud_market_prices_latest_dates(core_tickers)
    
    # Fallback/Supplemental check from local CSV files
    ...
    dates_set = set(ticker_dates.get(t, "2026-08-13") for t in core_tickers if t in ticker_dates)
    is_synced = len(dates_set) <= 1
    max_d = max(dates_set) if dates_set else "2026-08-13"
    
    with st.sidebar.expander(f"📡 各标的行情对齐监控 {'🟢 齐平' if is_synced else '⚠️ 存在差异'} ({max_d})", expanded=False):
        st.caption("检查各标的云端/本地 K 线最新对齐日期:")
        cols_m = st.sidebar.columns(2)
        for idx, t in enumerate(core_tickers):
            d_val = ticker_dates.get(t, "2026-08-13")
            col_m = cols_m[idx % 2]
            col_m.markdown(f"• **{t}**: `{d_val}`")
except Exception as err:
    st.sidebar.caption(f"📡 行情对齐监控: `{err}`")
...
with st.sidebar.form("us_trade_form"):
    ...
    t_price = st.number_input("成交单价 ($/股)", value=100.0, step=0.01, format="%.4f")
    ...
    if us_submitted:
        ...
        if pwd_hash != HK_IPO_PWD_HASH:
            ...
            execute_live_us_trade(t_date_str, t_ticker, action_code, t_price, t_shares, t_fee, layer_tag, t_reason)
            st.cache_data.clear()
            st.sidebar.success(f"✅ 已成功登记 {t_date_str} {action_code} {t_shares:.6f} 股 {t_ticker} (${t_price:.4f}/股)！页面数据已实时更新！")
            st.rerun()
    ...
    for idx, act in enumerate(actions, 1):
        ...
        signals_html_items.append(f"""
            <div style="display: flex; justify-content: space-between; align-items: center; background: rgba(0,0,0,0.25); padding: 5px 8px; border-radius: 8px;">
                <span>信号 {idx}: <strong style="color:{dir_color};">{act_dir} [{tkr}]</strong></span>
...
<div class="card-box">
    ...
    <div style="background: rgba(255,255,255,0.03); padding: 12px; border-radius: 16px; border: 1px solid rgba(255,255,255,0.06);">
        <div style="display: flex; justify-content: space-between; align-items: center;">
            ...
            <span style="font-size: 0.72rem; color: #6EE7B7; background: rgba(16, 185, 129, 0.15); border: 1px solid rgba(16, 185, 129, 0.3); padding: 1px 7px; border-radius: 8px; font-weight: bold;">实盘 {live_days} 天</span>
        </div>
        <div style="font-size: 2.5rem; font-weight: 900; color: #10B981; line-height: 1.1; margin: 4px 0;">+{total_live_ret_pct:.2f}%</div>
        <div style="font-size: 0.75rem; color: #CBD5E1;">累计总收益率</div>
